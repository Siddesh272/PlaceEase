            ...
            if (response.status === 200) {
                response
                    ...
                    .then(function () {
                        ...
                        if (layout != null) {
                            Array.from(
                                layout.querySelectorAll(".item-link")
                            ).forEach((link) => {
    ...
    fetch(url, {
        ...
        .then((data) => {
            ...
            if (data.success) {
                ...
                throw new Error(
                    data.message != undefined ? data.message : "Failed"
                );
            }
        })
        .catch((err) => {
            ...
            if (count < 1 && v.length >= min) {
                ...
                runSearch(rx, res, v, 1, (count) => {
                    if (count < 1) {
                        res.innerHTML = "";
                        res.appendChild(
                            ce("div", "item noresult", "", "No results")
                        );
                    }
                });
            }
...
function resizeSearch() {
    resetSearch(0);
    var i = _("#searchcontainer").children[0],
        o = _("#searchcontainermin").children[0];
    if (isMinScreen()) {
                ...
                .then((searchItems) => {
                    ...
                    searchItems.forEach((page) => {
                        ...
                        if (rx.test(k)) {
                            ...
                            it.setAttribute(
                                "target",
                                page.newTab ? "_blank" : "_self"
                            );
                            if (page.inlineTab) {
                                it.addEventListener(
                                    "click",
                                    openSearchedItemInTab.bind(
                                        event,
                                        it,
                                        page.url
                                    )
