...
const validateEmail = (email) => /\S+@student.sfit.ac.in/.test(email);
window.onload = () => {
    body = document.body;
...
function resetPasswordAndMessages(event) {
    const button = event.currentTarget;
    button
        .getAttribute("buttonTarget")
        .split(";")
        .forEach((x) => {
            _(`#${x}`).innerHTML = hiddenPassword;
        });
    button
        .getAttribute("resetTarget")
        .split(";")
        .forEach((x) => {
            _(`#${x}`).type = "password";
        });
...
function register() {
    ...
    if (!validateEmail(u)) {
        setMessage(
            registerEmail,
            "register",
            "Email with '@student.sfit.ac.in' domain allowed",
            "email",
            "error"
        );
        return;
                ...
                .then((data) => {
                    if (data.success === true) {
                        ...
                        try {
                            ...
                            var box =
                                _("#next-login-button").getBoundingClientRect();
                            var customEvent = {
