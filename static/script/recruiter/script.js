...
function addSkill(event) {
    ...
    if (text == "") {
        ...
        text.split(",")
            ...
            .forEach((ex) => {
                appendSkill(ex, target, main, inp);
            });
        textbox.value = "";
    }
...
function closeSkill(event) {
    ...
    p.removeChild(c);
    recheckSkillValues(
        _(`#${p.getAttribute("data-input")}`),
        _(`#${p.getAttribute("data-target")}`)
    );
}
function recheckSkillValues(input, target) {
    input.value = Array.from(target.children)
        .map((skill) => skill.children[0].innerHTML)
        .join(";");
}
function skillExists(value, target) {
    return Array.from(target.children).some(
        (shell) => shell.children[0].innerHTML == value
    );
}
...
function scheduleJobPost(event) {
    ...
    if (new Date(date) - 0 < new Date(dc.min) - 0) {
        message.innerHTML = `Select Date after ${new Date(dc.min)
            .toString()
            .split(" ")
            .splice(1, 3)
            .join(" ")}`;
...
function acceptSchedule(event) {
    ...
    button.classList.add("progress");
    fetch(
        `./dashboard/schedule/${event.currentTarget.getAttribute(
            "data-email"
        )}/approve`,
        {
            method: "POST",
        ...
        .then((data) => {
            if (data.success) {
                ...
                if (_(".schedule") == null) {
                    _(
                        ".scheduler"
                    ).innerHTML = `<div class="schedule">Nothing pending</div>`;
                }
...
function dumpData(url, event, code = null) {
    ...
    if (
        button.classList.contains("progress") ||
