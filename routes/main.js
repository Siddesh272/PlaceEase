...
route.get("/home", async (req, res) => {
    ...
    await user
        .initialize()
        .then((data) => {
            (isLoggedIn = data.isLoggedIn), (type = data.type);
        })
        .catch((error) => {
            console.log(error.message);
            (isLoggedIn = false), (type = "guest");
        })
        .finally(() => {
            res.render("home", {
                isLoggedIn,
...
route.post("/login", (req, res) => {
    ...
    if (!parseError) {
        //else part of try catch (no parse error)
    ...
    user.getUserData(req.body.email, {
        ...
        .catch((err) => {
            ...
            res.json({
                success: false,
                message:
                    err.name == "customError" ? err.message : "Unknown Error",
            });
        });
});
...
route.post("/reset/password", async (req, res) => {
    ...
    MongoClient.connect(DB_CONNECTION_URL, {
        ...
        .then(async (mongo) => {
            const db = await mongo.db(config.DB_SERVER.DB_DATABASE);
            return db.collection("user_data").updateOne(
                {
                    email,
                ...
                if (req.files != undefined) {
                    if (req.files.image != undefined && req.body.title != "") {
                        var fname = `${__dirname}/../data/gallery/${
                            req.body.title
                        }.${req.files.image.name.split(".").pop()}`;
    ...
    await user
        ...
        .then(async (data) => {
            ...
            await MongoClient.connect(DB_CONNECTION_URL, {
                ...
                getResultFromCursor(stat).then((statistics) => {
    ...
    await user
        ...
        .then(async (data) => {
            ...
            await MongoClient.connect(DB_CONNECTION_URL, {
                ...
                getResultFromCursor(coordinators).then((coordinators) => {
