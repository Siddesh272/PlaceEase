        ...
        .then(async (data) => {
            ...
            if (userData.result.data.job != undefined) {
                if (userData.result.data.job.schedule != undefined) {
                    recruiteraccepted =
                        userData.result.data.job.schedule.recruiteraccepted;
                    adminaccepted =
                        userData.result.data.job.schedule.adminaccepted;
                }
            }
        ...
        .then(async (data) => {
            ...
            if (userData.result.data != undefined) {
                ...
                if (jobPosted) {
                    ...
                    var appliedStudents = userData.result.data.job.applied.map(
                        (x) => {
                            return { email: x };
                        }
                    );
