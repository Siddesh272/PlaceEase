                ...
                },
                {
                    ...
                    $push: {
                        messages: {
                            ...
                            name: "Admin",
                            message:
                                "Your schedule request has been accepted to conduct on the requested date. Go to Schedule Section to see details.",
                            type: "general",
                            date: new Date(),
                        },
                    },
...
admin.post("/dashboard/:tab", async (req, res) => {
    ...
    await user
        ...
        .then(async (data) => {
            ...
            var notifications = [];
            if (req.params.tab == "main") {
                //notifications are only used in main page
                var userData = await user.getUserData(data.user, {
                    messages: 1,
                });
                if (!userData.success) {
